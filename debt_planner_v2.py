...
def plan_today(debts: list[Debt], bills: list[Bill], balance: float, buffer: float,
    ...
    2. safe_today = balance - buffer - reserved obligations. This is a CEILING,
       not the recommendation -- it's the most you could send without dipping
       into the buffer or the money owed to upcoming bills.
    3. Recommend sending your DEFINED extra_payment to the target debt, capped
       by safe_today. So you get a sane, fixed amount (e.g. $300) rather than
       "throw every spare dollar at it." If even that doesn't fit, recommend
       what does and flag that the rest frees up after payday.
    4. Flag anything due *today* so you don't miss it.
    5. If safe_today <= 0, recommend holding and report when funds free up.

    extra_payment defaults to infinity, which reproduces the old "send all
    safe surplus" behavior for any caller that doesn't set it.
    """
    obligations = []
    for d in debts:
        if d.balance <= 0:
    ...
    # The recommendation is your DEFINED extra payment, capped by what's safe.
    # safe_today is the ceiling; extra_payment is the target. Take the lesser.
    ...
    # Cascade the recommended amount onto debt targets (so if it would clear the
    # top target, the remainder rolls to the next one).
    extra = []
...
def main() -> None:
    ...
    plan = plan_today(debts, bills, balance, config["buffer"], today, payday,
                      ...
                      config.get("extra_payment", float("inf")))
    ...
    if plan["extra"]:
        for e in plan["extra"]:
            print(f"  SEND EXTRA: {money(e['amount'])} to {e['name']} "
                  f"(your {strategy} target)")
        if not plan["extra_affordable"] and plan["extra_payment"] is not None:
            print(f"  (note: your set extra payment is {money(plan['extra_payment'])}, but only")
            print(f"   {money(plan['safe_today'])} is safe today; the rest frees up {payday:%b %d}.)")
